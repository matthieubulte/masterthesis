...
\begin{theorem} \label{thm-edgeworth}
    ...
    \end{equation*}
    Assume that the characteristic function $\zeta$ of $P$ is $L^q(\Rp)$ for some $q \in [1, n)$. Then the Edgeworth series $e_k(y; \kappa)$ constructed by only keeping the first $k$ cumulants of $P$ in Equation (\ref{eq-edgeworth-full}) approximates $f_Y$, the density of $Y$, with an error of order $o(n^{1 - k/2})$.
\end{theorem}
