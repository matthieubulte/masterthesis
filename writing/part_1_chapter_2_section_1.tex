...
\begin{remark}
    ...
    \begin{equation*}
        \kappa_s = D^s K(0) 
        = \frac{\d^k}{\d x_{s_1} \ldots \d x_{s_k}} \log \zeta_X(-it) \bigg|_{t=0}
        = (-i)^{k} D^s \log \zeta_X(0).
    \end{equation*}
\end{remark}
...
\end{align*}
One can see that the summation over multiple index sets $s_k$ and index set lengths $r_k$ is redundant, and we can simplify the notation by introduction pseudo-moments $\alpha^*_s$ such that
\begin{equation*}
    \zeta(t) = 
    \sum_{j=0}^\infty 
    \sum_{s \in S(j)}
    \alpha^*_s (-1)^j \frac{\xi(t) (-i)^{j} t^{s}}{j!}.
\end{equation*}
By Lemma \ref{lemma-fourier-derivative}, we recognize the Fourier transform of derivatives of the density $q$ of $Q$
\begin{equation*}
...
\begin{align*}
    ...
    \right\},
\end{align*} 
where $h_s(\cdot; \mu_P, \Sigma_P)$ are a mutivariate generalization of the Hermite polynomials
\begin{equation*}
    h_s(x; \mu_P, \Sigma_P) = (-1)^j D^s q(x) / q(x).
\end{equation*}
