...
\end{equation}
A convenient choice for $Q$ is the multivariate Normal distribution $\mathcal{N}_p(0, \mathbb{1}_p)$. Then, from \ref{ex-cumulants-mvn}, we then have that $\alpha_s = 0$ and so $\alpha^*_s = 0$ for $s \in S(j)$, $j= 1, 2$. Furthermore, we also have that for $s \in S(j), j > 2$, $\alpha_s = \kappa_s$. Using this in (\ref{eq-edge-abstract}) gives
\begin{align}
    f(x) 
