...
\begin{theorem} \label{thm-edgeworth}
    ...
    \end{equation*}
    Let $e_k(y; \kappa)$ be the Edgeworth series, constructed by only keeping terms order up to $O(n^{1-k/2})$ in (\ref{eq-edgeworth-full}). Then, if the density $f_Y$ of $Y$ exists, $e_k(y; \kappa)$ approximates $f_Y$ with a uniform error of order $o(n^{(1 - k)/2})$.
\end{theorem}
