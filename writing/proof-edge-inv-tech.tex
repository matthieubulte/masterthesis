\begin{proof}
    ...
    \end{equation*}
    We can then rewrite $\zeta(t n^{-1/2})^n$ as follows,
    \begin{equation*}
    ...
    \end{align*}
    Next, we find suitable upper bounds on the right hand side of the above inequality. First note that both $u$ and $v$ have continuous derivatives in 0 of order up to $k$. Starting with $u^*$, let $s \in S_p(m)$ for $3 \leq m < k$, then
    \begin{align*}
        D^s u^*(0)
    ...
    \end{align*}
    where the derivative of the 2-norm of $t$ is 0 since $|s| = m \geq 3$. Similarly, we can compute the derivatives of $v^*$,
    \begin{align*}
        D^s v^*(0)
    ...
    \begin{equation*}
        ...
        = i^m\kappa_s,
    \end{equation*}
    where the last equality holds since $\kappa_s = \kappa_{s'}$ for all permutation $s'$ of $s$. 
    Therefore all derivatives of order up to $k$ of $u^* - v^*$ (and hence also $u - v$) exist in 0 and are all equal to 0. Thus, there exists $\delta > 0$ such that for all $t \in \Rp$ with $\norm{t}_2 \leq \delta$,
    \begin{equation*}
