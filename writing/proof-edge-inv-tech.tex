\begin{proof}
    The idea of this proof is to rewrite the left hand side of the (\ref{eq-edge-bound}) to be able to use Lemma \ref{lemma-series} and find suitable upper bounds on the remaining quantities. To that end, we define $u(t) = n u^*(t n^{-1/2})$ and $v(t) = nv^*(t n^{-1/2})$ where $u^*(t) = \log \zeta(t) + \frac{1}{2}\norm{t}_2^2$ and 
    \begin{equation*}
    ...
    \end{equation*}
    By Lemma \ref{lemma-series}, we can bound the left hand side of (\ref{eq-edge-bound}) and have
    \begin{align*}
